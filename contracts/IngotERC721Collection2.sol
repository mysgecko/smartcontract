...
contract IngotERC721Collection2 is ERC721URIStorage, ERC721Royalty, ReentrancyGuard, IIngotERC721Collection {
     ...
     * 2: only whitelist and owner can mint
     */
    ...
    modifier onlyHolder(uint256 tokenId) {
        require(msg.sender == ownerOf(tokenId), "Only the owner can call this function");
        _;
    }

    constructor(string memory _colName, string memory _colSymbol) ERC721(_colName, _colSymbol)  {
        _owner = tx.origin;
        _nextTokenId++;
    }
    ...
    function initialize(string memory _colName, string memory _colSymbol, string memory _baseTokenURI,
        ...
        _name = _colName;
        _symbol = _colSymbol;
        if(_nextTokenId == 0) _nextTokenId = _nextTokenId + 1;
        isLocked = true;
    }
    ...
    function normalMintNFTs(bool _isSafeMint,
        ...
        for (uint32 i = 0; i < _quantity; i++) {
            ...
            _setTokenRoyalty(newTokenId, _owner, _royaltyPercentInx100);
